...
public class FinancialSummaryService {
    ...
    public FinancialSummaryDto getSummary(LocalDate startDate, LocalDate endDate) {
        ...
        long revenueCents = transactionRepository.sumConfirmedRevenueCentsByOrganizationAndPeriod(orgId, start, end);
        long expenseCents = transactionRepository.sumConfirmedExpenseCentsByOrganizationAndPeriod(orgId, start, end);
        long pending = transactionRepository.countByOrganizationIdAndStatus(orgId, FinancialTransaction.TransactionStatus.PENDING);
    ...
    private List<FinancialSummaryDto.CategoryBreakdown> breakdown(Long orgId, FinancialTransaction.TransactionType type,
        ...
        return transactionRepository.sumConfirmedByCategoryAndPeriod(orgId, type, start, end).stream()
                .map(row -> {
                    ...
                    String name = categoryId != null
                            ? categoryRepository.findById(categoryId).map(c -> c.getName()).orElse("Sem categoria")
                            : "Sem categoria";
